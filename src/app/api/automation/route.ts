...
export async function POST(request: Request) {
    try {
        ...
        if (action === 'update') {
            const { niches, style, platforms, types, frequency, bulkCount, rotateNiches } = body;
            await store.updateAutomationConfig({ niches, style, platforms, types, frequency, bulkCount, rotateNiches });
            const updated = await store.getAutomationConfig();
            return NextResponse.json({ automation: updated });
        }
        ...
        if (action === 'run') {
            ...
            await store.addActivity('automation', 'Bulk Pipeline Run Complete', `Generated ${results.length} items with custom branding.`);
            return NextResponse.json({ automation: await store.getAutomationConfig(), results });
        }
